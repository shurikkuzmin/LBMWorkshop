...
void update_bounce_back()
{
	...
	for(int iX=0;iX<NX;iX++)
	{
		...
		f2[iX][0][2]=f2[iX][1][4]+2.0*weights[2]*3.0*(-u_wall*cx[2]);
		f2[iX][0][5]=f2[iXtop][1][7]+2.0*weights[5]*3.0*(-u_wall*cx[5]);
		f2[iX][0][6]=f2[iXbottom][1][8]+2.0*weights[6]*3.0*(-u_wall*cx[6]);

		f2[iX][NY-1][4]=f2[iX][NY-2][2]+2.0*weights[4]*3.0*(u_wall*cx[4]);
		f2[iX][NY-1][7]=f2[iXbottom][NY-2][5]+2.0*weights[7]*3.0*(u_wall*cx[7]);
		f2[iX][NY-1][8]=f2[iXtop][NY-2][6]+2.0*weights[8]*3.0*(u_wall*cx[8]);
		...
		rho[iX][0]=1.0;
		rho[iX][NY-1]=1.0;
		ux[iX][0]=-u_wall;ux[iX][NY-1]=u_wall;
...
void collide_bulk(int counter)
{
	...
	if (counter>NFORCE)
	{
		for(int iX=0;iX<NX;iX++)
		{
			phase[iX][0]=phase[iX][1];
			phase[iX][NY-1]=phase[iX][NY-2];
		}
	}
