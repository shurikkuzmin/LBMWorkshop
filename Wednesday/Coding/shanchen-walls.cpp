...
int main(int argc, char** argv)
{
	...
	{	
		if ((i/nx-ny+radius+4)*(i/nx-ny+radius+4)+(i%nx-nx/2.0)*(i%nx-nx/2.0)<=radius*radius)
		{
			rho[i]=rhol;
		}
		else 
	...
	//Additional loop for stabilization
	
	for (int timecounter=0; timecounter<=300;timecounter++) 
	{
		...
		if (timecounter%noutput==0)
		{
			...
			len<<timecounter;
			imagestream << "heightstab"<<std::string(5-len.str().size(),'0')<<timecounter<<".dat";
	...
	for (int timecounter=0; timecounter<=nsteps;timecounter++) 
	{
		...
		//Putting density walls
		for (int i=0;i<nx;i++)
		{
			rho[i]=rho_wall;
			rho[nx*(ny-1)+i]=rho_wall;
		}
		...
		for (int iY=1; iY<ny-1; iY++) 
			...
			//Streaming for the BB nodes
			for(int iX=0;iX<nx;iX++)
			{
		    	//top wall 
		        f2[9*(iX+(ny-2)*nx)+4]=f[9*(iX+(ny-2)*nx)+2];
		    	f2[9*(iX+(ny-2)*nx)+7]=f[9*(iX+(ny-2)*nx)+5];
		    	f2[9*(iX+(ny-2)*nx)+8]=f[9*(iX+(ny-2)*nx)+6];
		    
		        //bottom wall
		        f2[9*(iX+nx)+2]=f[9*(iX+nx)+4];
		    	f2[9*(iX+nx)+5]=f[9*(iX+nx)+7];
		    	f2[9*(iX+nx)+6]=f[9*(iX+nx)+8];
			}
